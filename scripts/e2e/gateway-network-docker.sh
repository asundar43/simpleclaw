...
IMAGE_NAME="simpleclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="simpleclaw-net-e2e-$$"
GW_NAME="simpleclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "SIMPLECLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "SIMPLECLAW_SKIP_CHANNELS=1" \
  -e "SIMPLECLAW_SKIP_GMAIL_WATCHER=1" \
  -e "SIMPLECLAW_SKIP_CRON=1" \
  -e "SIMPLECLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
