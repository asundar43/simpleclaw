#!/usr/bin/env bash
# ClawDock - Docker helpers for SimpleClaw
# Inspired by Simon Willison's "Running SimpleClaw in Docker"
# https://til.simonwillison.net/llms/simpleclaw-docker
#
# Installation:
...
# Common paths to check for SimpleClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/simpleclaw"
  "${HOME}/workspace/simpleclaw"
  "${HOME}/projects/simpleclaw"
  "${HOME}/dev/simpleclaw"
  "${HOME}/code/simpleclaw"
  "${HOME}/src/simpleclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SimpleClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/simpleclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SimpleClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/simpleclaw/simpleclaw.git ~/simpleclaw"
    echo "  cd ~/simpleclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SIMPLECLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d simpleclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart simpleclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f simpleclaw-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec simpleclaw-gateway \
    bash -c 'echo "alias simpleclaw=\"./simpleclaw.mjs\"" > /tmp/.bashrc_simpleclaw && bash --rcfile /tmp/.bashrc_simpleclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm simpleclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build simpleclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SIMPLECLAW_GATEWAY_TOKEN=$token" simpleclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" simpleclaw-gateway \
    bash -c './simpleclaw.mjs config set gateway.remote.token "$TOKEN" && ./simpleclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec simpleclaw-gateway \
    bash -c "./simpleclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart simpleclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm simpleclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec simpleclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'simpleclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SimpleClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (simpleclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to simpleclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.simpleclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'simpleclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'simpleclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.simpleclaw.ai${_CLR_RESET}"
  echo ""
}
