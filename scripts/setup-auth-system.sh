#!/bin/bash
# Setup SimpleClaw Auth Management System
...
echo "=== SimpleClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'simpleclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SimpleClaw message: Send warning via SimpleClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/simpleclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/simpleclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/simpleclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now simpleclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status simpleclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
