...
# Checks both Claude Code and SimpleClaw auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
SIMPLECLAW_AUTH="$HOME/.simpleclaw/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    simpleclaw models status --json 2>/dev/null || true
}
...
check_simpleclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$SIMPLECLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    simpleclaw_status=$(check_simpleclaw_auth 2>/dev/null || true)

    claude_expires=0
    simpleclaw_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        simpleclaw_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        simpleclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$SIMPLECLAW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$simpleclaw_status" \
        --arg be "$simpleclaw_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            simpleclaw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$simpleclaw_status" == EXPIRED* ]] || [[ "$simpleclaw_status" == MISSING* ]]; then
        echo "SIMPLECLAW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$simpleclaw_status" == EXPIRING* ]]; then
        echo "SIMPLECLAW_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "SimpleClaw Auth (~/.simpleclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$SIMPLECLAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$SIMPLECLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'simpleclaw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active simpleclaw >/dev/null 2>&1; then
    echo -e "SimpleClaw service: ${GREEN}running${NC}"
else
    echo -e "SimpleClaw service: ${RED}NOT running${NC}"
fi
