...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/simpleclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SimpleClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/simpleclaw/scripts/mobile-reauth.sh'" --id simpleclaw-auth
        ;;
    *)
