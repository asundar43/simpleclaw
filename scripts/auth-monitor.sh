...
#
# Suggested cron: */30 * * * * /home/admin/simpleclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SimpleClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., simpleclaw-alerts)
...
STATE_FILE="$HOME/.simpleclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SimpleClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use simpleclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SimpleClaw to $NOTIFY_PHONE..."
            simpleclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SimpleClaw is down. Run: ssh l36 '~/simpleclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
