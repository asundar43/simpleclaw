 ...
 * `simpleclaw update`. Use it to verify the completion installation flow
...
\`simpleclaw update\`, \`simpleclaw doctor\`, and \`simpleclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SimpleClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`simpleclaw completion --install\` later to enable.`));
    return;
  }
