...
show_usage() {
    cat << EOF
clawlog - SimpleClaw Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View SimpleClaw logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    SimpleClaw logs flow through the macOS unified log (subsystem: ai.simpleclaw).
