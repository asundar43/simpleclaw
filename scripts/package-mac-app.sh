...
# Build and bundle SimpleClaw into a minimal .app we can open.
# Outputs to dist/SimpleClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SimpleClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SimpleClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.simpleclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/simpleclaw/simpleclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SimpleClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SimpleClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SimpleClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SimpleClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SimpleClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SimpleClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SimpleClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SimpleClaw/Resources/SimpleClaw.icns" "$APP_ROOT/Contents/Resources/SimpleClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SimpleClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SimpleClawKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SimpleClawKit_SimpleClawKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SimpleClawKit_SimpleClawKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SimpleClawKit_SimpleClawKit.bundle"
else
  echo "WARN: SimpleClawKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SimpleClaw"
killall -q SimpleClaw 2>/dev/null || true
