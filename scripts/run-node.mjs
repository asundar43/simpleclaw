...
const shouldBuild = (deps) => {
  if (deps.env.SIMPLECLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SIMPLECLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[simpleclaw] ${message}\n`);
};

const runSimpleClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["simpleclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runSimpleClaw(deps);
  }
