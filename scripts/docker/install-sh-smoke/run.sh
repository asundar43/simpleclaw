...
INSTALL_URL="${SIMPLECLAW_INSTALL_URL:-https://simpleclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SIMPLECLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SIMPLECLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="simpleclaw"
PACKAGE_NAME="${SIMPLECLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SIMPLECLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SIMPLECLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SIMPLECLAW_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
