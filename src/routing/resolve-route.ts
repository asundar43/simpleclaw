...
function listAgents(cfg: SimpleClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: SimpleClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SimpleClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<SimpleClawConfig, EvaluatedBindingsCache>();
