...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.SIMPLECLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service SIMPLECLAW_GATEWAY_TOKEN does not match gateway.auth.token in simpleclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `simpleclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
