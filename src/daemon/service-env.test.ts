...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SIMPLECLAW_GATEWAY_PORT).toBe("18789");
    expect(env.SIMPLECLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.SIMPLECLAW_SERVICE_MARKER).toBe("simpleclaw");
    expect(env.SIMPLECLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SIMPLECLAW_SERVICE_VERSION).toBe("string");
    expect(env.SIMPLECLAW_SYSTEMD_UNIT).toBe("simpleclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SIMPLECLAW_LAUNCHD_LABEL).toBe("ai.simpleclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SIMPLECLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SIMPLECLAW_SYSTEMD_UNIT).toBe("simpleclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SIMPLECLAW_LAUNCHD_LABEL).toBe("ai.simpleclaw.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".simpleclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SIMPLECLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".simpleclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SIMPLECLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".simpleclaw"));
  });

  it("uses SIMPLECLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SIMPLECLAW_STATE_DIR: "/var/lib/simpleclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/simpleclaw"));
  });

  it("expands ~ in SIMPLECLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SIMPLECLAW_STATE_DIR: "~/simpleclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/simpleclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SIMPLECLAW_STATE_DIR: "C:\\State\\simpleclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\simpleclaw");
  });
});
