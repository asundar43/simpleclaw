...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.SIMPLECLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.SIMPLECLAW_STATE_DIR;
  const configPath = env.SIMPLECLAW_CONFIG_PATH;
  ...
  return {
    ...
    SIMPLECLAW_PROFILE: profile,
    SIMPLECLAW_STATE_DIR: stateDir,
    SIMPLECLAW_CONFIG_PATH: configPath,
    SIMPLECLAW_GATEWAY_PORT: String(port),
    SIMPLECLAW_GATEWAY_TOKEN: token,
    SIMPLECLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SIMPLECLAW_SYSTEMD_UNIT: systemdUnit,
    SIMPLECLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SIMPLECLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    SIMPLECLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    SIMPLECLAW_STATE_DIR: stateDir,
    SIMPLECLAW_CONFIG_PATH: configPath,
    SIMPLECLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    SIMPLECLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    SIMPLECLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    SIMPLECLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    SIMPLECLAW_LOG_PREFIX: "node",
    SIMPLECLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    SIMPLECLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    SIMPLECLAW_SERVICE_VERSION: VERSION,
  };
}
