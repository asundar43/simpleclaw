...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SIMPLECLAW_HOME for tilde expansion", () => {
    vi.stubEnv("SIMPLECLAW_HOME", "/srv/simpleclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/simpleclaw-home", "cron", "jobs.json"));
  });
});
