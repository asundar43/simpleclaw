...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "simpleclaw-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".simpleclaw", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SimpleClawConfig> = {},
): SimpleClawConfig {
  const base: SimpleClawConfig = {
    agents: {
      defaults: {
