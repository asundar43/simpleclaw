...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "simpleclaw_cmdarg", "arg-menu action");
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("simpleclaw_cmdarg")).toBe(true);
    expect(testHarness.options.has("simpleclaw_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("simpleclaw_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("simpleclaw_cmdarg");
    expect(blockId).toContain("simpleclaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("simpleclaw_cmdarg_ext:".length);
