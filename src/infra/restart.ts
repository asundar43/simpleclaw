...
export function triggerSimpleClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SIMPLECLAW_SYSTEMD_UNIT,
        process.env.SIMPLECLAW_PROFILE,
      );
  ...
  const label =
    process.env.SIMPLECLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SIMPLECLAW_PROFILE);
