...
import {
  POSIX_SIMPLECLAW_TMP_DIR,
  resolvePreferredSimpleClawTmpDir,
} from "./tmp-simpleclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredSimpleClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `simpleclaw-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredSimpleClawTmpDir({
    accessSync,
...
describe("resolvePreferredSimpleClawTmpDir", () => {
  it("prefers /tmp/simpleclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SIMPLECLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/simpleclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SIMPLECLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/simpleclaw when /tmp/simpleclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/simpleclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/simpleclaw is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/simpleclaw is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/simpleclaw is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
