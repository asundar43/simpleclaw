...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SIMPLECLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SIMPLECLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SIMPLECLAW_APNS_TEAM_ID and SIMPLECLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SIMPLECLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SIMPLECLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SIMPLECLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SIMPLECLAW_APNS_PRIVATE_KEY_P8 or SIMPLECLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SIMPLECLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createSimpleClawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    simpleclaw: createSimpleClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
