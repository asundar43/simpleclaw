...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SimpleClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SimpleClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: SimpleClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: SimpleClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SimpleClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SimpleClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SimpleClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
