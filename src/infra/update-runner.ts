...
const DEFAULT_PACKAGE_NAME = "simpleclaw";
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "simpleclaw-update-preflight-"),
      );
    ...
    const doctorEntry = path.join(gitRoot, "simpleclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("simpleclaw doctor", doctorArgv, gitRoot, { SIMPLECLAW_UPDATE_IN_PROGRESS: "1" }),
    );
