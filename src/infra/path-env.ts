...
function candidateBinDirs(opts: EnsureSimpleClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `simpleclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "simpleclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SIMPLECLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "simpleclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `simpleclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSimpleClawCliOnPath(opts: EnsureSimpleClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SIMPLECLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
