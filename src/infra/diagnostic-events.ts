...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __simpleclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__simpleclawDiagnosticEventsState) {
    globalStore.__simpleclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__simpleclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: SimpleClawConfig): boolean {
