...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred SimpleClaw tmp root", async () => {
    const tmpRoot = resolvePreferredSimpleClawTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideSimpleClaw = path.join(
        os.tmpdir(),
        "outside-simpleclaw",
        "test-media.png",
      );
      await expect(
        runMessageAction({
          ...
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsideSimpleClaw,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
