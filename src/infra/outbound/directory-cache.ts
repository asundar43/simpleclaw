...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SimpleClawConfig | null = null;
  ...
  get(key: string, cfg: SimpleClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SimpleClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: SimpleClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: SimpleClawConfig): void {
