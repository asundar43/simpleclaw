...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SIMPLECLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.SIMPLECLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveSimpleClawPackageRoot } = await import("./simpleclaw-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveSimpleClawPackageRoot).mockResolvedValue("/opt/simpleclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/simpleclaw/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
