...
let ensureSimpleClawCliOnPath: typeof import("./path-env.js").ensureSimpleClawCliOnPath;

describe("ensureSimpleClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "SIMPLECLAW_PATH_BOOTSTRAPPED",
    "SIMPLECLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureSimpleClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling simpleclaw exists", () => {
    const tmp = abs("/tmp/simpleclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "simpleclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.SIMPLECLAW_PATH_BOOTSTRAPPED;

    ensureSimpleClawCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.SIMPLECLAW_PATH_BOOTSTRAPPED = "1";
    ensureSimpleClawCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/simpleclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "simpleclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/simpleclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "simpleclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "simpleclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/simpleclaw-path/case-linuxbrew");
