...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SIMPLECLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*simpleclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "simpleclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "simpleclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "simpleclaw-gw");
  ...
  records.push(`_simpleclaw-gw._tcp IN PTR ${instanceLabel}._simpleclaw-gw._tcp`);
  records.push(`${instanceLabel}._simpleclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._simpleclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; simpleclaw-content-hash: ${contentHash}\n${contentBody}`;
}
