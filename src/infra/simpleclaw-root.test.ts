...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__simpleclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "simpleclaw-root");
...
describe("resolveSimpleClawPackageRoot", () => {
  let resolveSimpleClawPackageRoot: typeof import("./simpleclaw-root.js").resolveSimpleClawPackageRoot;
  let resolveSimpleClawPackageRootSync: typeof import("./simpleclaw-root.js").resolveSimpleClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveSimpleClawPackageRoot, resolveSimpleClawPackageRootSync } =
      await import("./simpleclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "simpleclaw");
    const pkgRoot = path.join(project, "node_modules", "simpleclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "simpleclaw" }));

    expect(resolveSimpleClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "simpleclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "simpleclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "simpleclaw" }));

    expect(resolveSimpleClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveSimpleClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-simpleclaw package roots", async () => {
    const pkgRoot = fx("not-simpleclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-simpleclaw" }));

    expect(resolveSimpleClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveSimpleClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveSimpleClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
