...
vi.mock("./simpleclaw-root.js", () => ({
  resolveSimpleClawPackageRoot: vi.fn(async () => null),
  resolveSimpleClawPackageRootSync: vi.fn(() => null),
}));
...
let openclawRoot: typeof import("./simpleclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openclawRoot = await import("./simpleclaw-root.js");
  });
  ...
  it("uses resolveSimpleClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/simpleclaw");
    (
      openclawRoot.resolveSimpleClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/simpleclaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "simpleclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "simpleclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-simpleclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/simpleclaw-bundle");
    (
      openclawRoot.resolveSimpleClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
