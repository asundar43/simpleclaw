...
(() => {
  ...
  const hasAndroid = () =>
    !!(
      (window.simpleClawCanvasA2UIAction &&
        typeof window.simpleClawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.simpleClawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("simpleclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SimpleClaw node canvas.");
      return;
    }
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SIMPLECLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SIMPLECLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SimpleClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
