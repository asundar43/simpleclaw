...
describe("canvas host state dir defaults", () => {
  it("uses SIMPLECLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("simpleclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("SimpleClaw Canvas");
      } finally {
        await handler.close();
      }
    });
