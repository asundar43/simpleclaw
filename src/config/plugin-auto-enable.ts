...
function isWhatsAppConfigured(cfg: SimpleClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: SimpleClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: SimpleClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SimpleClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: SimpleClawConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: SimpleClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SimpleClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: SimpleClawConfig, pluginId: string): SimpleClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
