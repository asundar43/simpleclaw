  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.simpleclaw/logs/simpleclaw.log" },
        hooks: {
          path: "~/.simpleclaw/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.simpleclaw/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".simpleclaw", "logs", "simpleclaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".simpleclaw", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".simpleclaw", "telegram.token"),
      );
