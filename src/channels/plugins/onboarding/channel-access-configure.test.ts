...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: SimpleClawConfig, policy: ChannelAccessPolicy) => SimpleClawConfig;
  resolveAllowlist: (params: { cfg: SimpleClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: SimpleClawConfig; resolved: TResolved }) => SimpleClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: SimpleClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: SimpleClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: SimpleClawConfig, policy: ChannelAccessPolicy): SimpleClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: SimpleClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: SimpleClawConfig; resolved: string[] }) => {
      calls.push("apply");
