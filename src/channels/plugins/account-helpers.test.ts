...
function cfg(accounts?: Record<string, unknown> | null): SimpleClawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as SimpleClawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as SimpleClawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as SimpleClawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as SimpleClawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as SimpleClawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as SimpleClawConfig)).toBe("default");
    });
  });
});
