...
const GOOGLE_LIVE_SINGLE_KEY = "SIMPLECLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SIMPLECLAW_LIVE_ANTHROPIC_KEY",
    listVar: "SIMPLECLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SIMPLECLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SIMPLECLAW_LIVE_${base}_KEY`;
