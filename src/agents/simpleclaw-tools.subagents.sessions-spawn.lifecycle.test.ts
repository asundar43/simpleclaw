...
} from "./simpleclaw-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.SIMPLECLAW_TEST_FAST;
  process.env.SIMPLECLAW_TEST_FAST = "1";
  return { previous };
});
...
describe("simpleclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.SIMPLECLAW_TEST_FAST;
      return;
    }
    process.env.SIMPLECLAW_TEST_FAST = fastModeEnv.previous;
  });
