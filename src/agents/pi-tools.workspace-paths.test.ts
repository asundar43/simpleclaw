...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("simpleclaw-ws-", async (workspaceDir) => {
      await withTempDir("simpleclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSimpleClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("simpleclaw-ws-", async (workspaceDir) => {
      await withTempDir("simpleclaw-override-", async (overrideDir) => {
        const tools = createSimpleClawCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("simpleclaw-ws-", async (workspaceDir) => {
      const cfg: SimpleClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createSimpleClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-simpleclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("simpleclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("simpleclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createSimpleClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
