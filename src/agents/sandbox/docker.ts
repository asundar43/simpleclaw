...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "simpleclaw.sandbox=1");
  args.push("--label", `simpleclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `simpleclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `simpleclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "simpleclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`simpleclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`simpleclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("simpleclaw sandbox recreate --all");
}
