...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: SimpleClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/simpleclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SimpleClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SimpleClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SimpleClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: SimpleClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: SimpleClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SimpleClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: SimpleClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/simpleclaw");
  });

  it("uses SIMPLECLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "simpleclaw-home");
    vi.stubEnv("SIMPLECLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as SimpleClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".simpleclaw", "workspace"));
  });
