...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureSimpleClawModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): SimpleClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): SimpleClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-workspace-"));
    const now = Date.now();
