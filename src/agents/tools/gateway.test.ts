...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    simpleclaw: process.env.SIMPLECLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.simpleclaw === undefined) {
      delete process.env.SIMPLECLAW_GATEWAY_TOKEN;
    } else {
      process.env.SIMPLECLAW_GATEWAY_TOKEN = envSnapshot.simpleclaw;
    }
  ...
  it("uses SIMPLECLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SIMPLECLAW_GATEWAY_TOKEN = "local-env-token";
