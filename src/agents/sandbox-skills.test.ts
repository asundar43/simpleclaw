...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "simpleclaw-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.SIMPLECLAW_BUNDLED_SKILLS_DIR = bundledDir;
