...
describe("resolveSimpleClawAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          SIMPLECLAW_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSimpleClawAgentDir();
  ...
  it("honors SIMPLECLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          SIMPLECLAW_STATE_DIR: undefined,
          SIMPLECLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSimpleClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when SIMPLECLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers SIMPLECLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          SIMPLECLAW_STATE_DIR: undefined,
          SIMPLECLAW_AGENT_DIR: primaryOverride,
