...
type CoreToolDefinition = {
  ...
  includeInSimpleClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const simpleClawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSimpleClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:simpleclaw": simpleClawTools,
