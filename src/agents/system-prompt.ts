...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `simpleclaw status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "SimpleClaw is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- simpleclaw gateway status",
    "- simpleclaw gateway start",
    "- simpleclaw gateway stop",
    "- simpleclaw gateway restart",
    "If unsure, ask the user to run `simpleclaw help` (or `simpleclaw gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## SimpleClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, SimpleClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
