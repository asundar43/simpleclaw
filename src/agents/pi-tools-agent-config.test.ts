...
import { createSimpleClawCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SimpleClawConfig) {
    return createSimpleClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SimpleClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SimpleClawConfig["agents"]>["list"]>[number]["tools"];
  }): SimpleClawConfig {
    return {
      tools: params.tools,
      agents: {
    ...
    const cfg: SimpleClawConfig = {
      ...
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/simpleclaw-restricted",
            tools: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createSimpleClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createSimpleClawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createSimpleClawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createSimpleClawCodingTools({
      config: cfg,
    ...
    const bobTools = createSimpleClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createSimpleClawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createSimpleClawCodingTools({
      config: cfg,
