...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SIMPLECLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SIMPLECLAW_HOME: "/srv/simpleclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/simpleclaw-home"), ".simpleclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".simpleclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("simpleclaw-workspace-");
