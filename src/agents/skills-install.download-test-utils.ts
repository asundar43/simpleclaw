...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("simpleclaw-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-skills-install-"));
  try {
