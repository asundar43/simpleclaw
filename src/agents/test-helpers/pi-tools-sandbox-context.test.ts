...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("simpleclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("simpleclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("simpleclaw-sbx-");
