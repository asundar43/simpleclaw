...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SIMPLECLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-subagent-"));
  ...
  it("uses isolated temp state when SIMPLECLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "simpleclaw-test-state"));
  });
});
