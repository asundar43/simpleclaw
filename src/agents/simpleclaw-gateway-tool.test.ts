...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createSimpleClawTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-test-"));

    try {
      await withEnvAsync(
        { SIMPLECLAW_STATE_DIR: stateDir, SIMPLECLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: simpleclaw --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/simpleclaw" } }\n}\n';
