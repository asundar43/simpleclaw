...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredSimpleClawTmpDir(), "simpleclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub SIMPLECLAW_STATE_DIR.
    // Also keep it outside the SimpleClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["SIMPLECLAW_STATE_DIR"]);
    process.env.SIMPLECLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "simpleclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredSimpleClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default SimpleClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default SimpleClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
