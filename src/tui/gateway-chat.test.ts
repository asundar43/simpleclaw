...
describe("resolveGatewayConnection", () => {
  ...
  it("uses SIMPLECLAW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ SIMPLECLAW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers SIMPLECLAW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ SIMPLECLAW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
