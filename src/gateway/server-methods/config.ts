...
import type { SimpleClawConfig } from "../../config/types.simpleclaw.js";
import {
...
import { loadSimpleClawPlugins } from "../../plugins/loader.js";
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: SimpleClawConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadSimpleClawPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadSimpleClawPlugins() already cache
