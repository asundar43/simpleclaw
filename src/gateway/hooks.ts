...
export function resolveHooksConfig(cfg: SimpleClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SimpleClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-simpleclaw-token"] === "string"
      ? req.headers["x-simpleclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
