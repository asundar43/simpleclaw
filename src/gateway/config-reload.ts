...
import type { SimpleClawConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: SimpleClawConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: SimpleClawConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: SimpleClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: SimpleClawConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: SimpleClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: SimpleClawConfig) => {
