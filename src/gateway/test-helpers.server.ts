...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "SIMPLECLAW_SKIP_BROWSER_CONTROL_SERVER",
  "SIMPLECLAW_SKIP_GMAIL_WATCHER",
  "SIMPLECLAW_SKIP_CANVAS_HOST",
  "SIMPLECLAW_BUNDLED_PLUGINS_DIR",
  "SIMPLECLAW_SKIP_CHANNELS",
  "SIMPLECLAW_SKIP_PROVIDERS",
  "SIMPLECLAW_SKIP_CRON",
  "SIMPLECLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SIMPLECLAW_STATE_DIR = path.join(tempHome, ".simpleclaw");
  delete process.env.SIMPLECLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.SIMPLECLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  ...
  process.env.SIMPLECLAW_SKIP_PROVIDERS = "1";
  process.env.SIMPLECLAW_SKIP_CRON = "1";
  process.env.SIMPLECLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.SIMPLECLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "simpleclaw-test-no-bundled-extensions")
    : "simpleclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "simpleclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".simpleclaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__simpleclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__simpleclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["SIMPLECLAW_GATEWAY_TOKEN"]);
  const prev = process.env.SIMPLECLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SIMPLECLAW_GATEWAY_TOKEN = fallbackToken;
  }
