...
const { createSimpleClawTools } = await import("../agents/simpleclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createSimpleClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSimpleClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SIMPLECLAW_GATEWAY_PORT", "SIMPLECLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.SIMPLECLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.SIMPLECLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.SIMPLECLAW_CONFIG_PATH;
      if (!configPath) {
        throw new Error("SIMPLECLAW_CONFIG_PATH missing in gateway test environment");
      }
