...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/simpleclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SimpleClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SIMPLECLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "simpleclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SIMPLECLAW_TAILNET_DNS?.trim();
