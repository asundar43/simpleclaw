...
let lastCreateSimpleClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/simpleclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSimpleClawToolsContext?.agentTo,
          agentThreadId: lastCreateSimpleClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSimpleClawTools: (ctx: Record<string, unknown>) => {
      lastCreateSimpleClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSimpleClawToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-simpleclaw-message-to": "channel:24514",
        "x-simpleclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
