...
type GetReplyFromConfigFn = (
  ...
  configOverride?: SimpleClawConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `simpleclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SIMPLECLAW_CONFIG_PATH = path.join(root, "simpleclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "simpleclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "simpleclaw-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadSimpleClawPlugins: () => pluginRegistryState.registry,
  };
});
