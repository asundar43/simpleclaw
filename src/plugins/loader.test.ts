...
import { loadSimpleClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `simpleclaw-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.SIMPLECLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "simpleclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.SIMPLECLAW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadSimpleClawPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadSimpleClawPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.SIMPLECLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.SIMPLECLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadSimpleClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadSimpleClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.SIMPLECLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ SIMPLECLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
