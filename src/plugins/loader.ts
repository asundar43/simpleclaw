...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SimpleClawPluginDefinition;
  register?: SimpleClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SimpleClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SimpleClawPluginDefinition;
...
export function loadSimpleClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSimpleClawPlugins({
    workspaceDir: options.workspaceDir,
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias
                ? { "simpleclaw/plugin-sdk": pluginSdkAlias, "openclaw/plugin-sdk": pluginSdkAlias }
                : {}),
              ...(pluginSdkAccountIdAlias
                ? {
                    "simpleclaw/plugin-sdk/account-id": pluginSdkAccountIdAlias,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SimpleClawPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as SimpleClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
