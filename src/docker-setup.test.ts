...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "simpleclaw-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SIMPLECLAW_GATEWAY_TOKEN: "test-token",
    SIMPLECLAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SIMPLECLAW_WORKSPACE_DIR: join(sandbox.rootDir, "simpleclaw"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SIMPLECLAW_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SIMPLECLAW_EXTRA_MOUNTS: undefined,
      SIMPLECLAW_HOME_VOLUME: "simpleclaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SIMPLECLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SIMPLECLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("SIMPLECLAW_HOME_VOLUME=simpleclaw-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("simpleclaw-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("simpleclaw-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SIMPLECLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SIMPLECLAW_CONFIG_DIR: configDir,
      SIMPLECLAW_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline SIMPLECLAW_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SIMPLECLAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SIMPLECLAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SIMPLECLAW_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SIMPLECLAW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SIMPLECLAW_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SIMPLECLAW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SIMPLECLAW_HOME_VOLUME must match");
  });
