...
type PromptDefaultModelResult = { model?: string; config?: SimpleClawConfig };
...
function resolveConfiguredModelRaw(cfg: SimpleClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SimpleClawConfig): string[] {
...
export function applyPrimaryModel(cfg: SimpleClawConfig, model: string): SimpleClawConfig {
...
export function applyModelAllowlist(cfg: SimpleClawConfig, models: string[]): SimpleClawConfig {
  const defaults = cfg.agents?.defaults;
