...
const ensureSimpleClawModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/simpleclaw-agent/auth.json" });
const resolveSimpleClawAgentDir = vi.fn().mockReturnValue("/tmp/simpleclaw-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/simpleclaw-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureSimpleClawModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/simpleclaw-agent");
  });
