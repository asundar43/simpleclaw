...
function resolveAgentName(cfg: SimpleClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: SimpleClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SimpleClawConfig): AgentSummary[] {
