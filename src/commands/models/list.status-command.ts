...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSimpleClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("simpleclaw models auth setup-token")}\` or \`${formatCliCommand("simpleclaw configure")}\`.`
          : `Run \`${formatCliCommand("simpleclaw configure")}\` or set an API key env var.`;
