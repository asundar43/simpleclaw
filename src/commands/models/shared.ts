...
export async function loadValidConfigOrThrow(): Promise<SimpleClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: SimpleClawConfig) => SimpleClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SimpleClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SimpleClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("simpleclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
