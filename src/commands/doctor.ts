...
function resolveMode(cfg: SimpleClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("SimpleClaw doctor");

  const root = await resolveSimpleClawPackageRoot({
  ...
  let cfg: SimpleClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("simpleclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("simpleclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("simpleclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("simpleclaw doctor --fix")}" to apply changes.`);
  }
