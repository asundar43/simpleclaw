...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SIMPLECLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SIMPLECLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SIMPLECLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("SIMPLECLAW_GATEWAY_PASSWORD");
  });
