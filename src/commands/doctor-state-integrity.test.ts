...
type EnvSnapshot = {
  HOME?: string;
  SIMPLECLAW_HOME?: string;
  SIMPLECLAW_STATE_DIR?: string;
  SIMPLECLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    SIMPLECLAW_HOME: process.env.SIMPLECLAW_HOME,
    SIMPLECLAW_STATE_DIR: process.env.SIMPLECLAW_STATE_DIR,
    SIMPLECLAW_OAUTH_DIR: process.env.SIMPLECLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: SimpleClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: SimpleClawConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "simpleclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.SIMPLECLAW_HOME = tempHome;
    process.env.SIMPLECLAW_STATE_DIR = path.join(tempHome, ".simpleclaw");
    delete process.env.SIMPLECLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.SIMPLECLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when SIMPLECLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.SIMPLECLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SimpleClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints simpleclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SimpleClawConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/simpleclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/simpleclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
