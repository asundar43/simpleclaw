...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.SIMPLECLAW_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.SIMPLECLAW_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from simpleclaw.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "SimpleClaw update wizard" : "SimpleClaw configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: SimpleClawConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.simpleclaw.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("simpleclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.SIMPLECLAW_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.SIMPLECLAW_GATEWAY_PASSWORD,
    });
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.SIMPLECLAW_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.SIMPLECLAW_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.SIMPLECLAW_GATEWAY_TOKEN;
