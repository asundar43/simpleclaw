...
const SEARCH_TOOL = "https://docs.simpleclaw.ai/mcp.SearchSimpleClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.simpleclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('simpleclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.simpleclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('simpleclaw docs "your query"')}`);
    }
    return;
  }
