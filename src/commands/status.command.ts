...
export async function statusCommand(
  ...
  runtime.log(theme.heading("SimpleClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`simpleclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(
      theme.muted(`Fallback: ${formatCliCommand("simpleclaw devices approve --latest")}`),
    );
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("simpleclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("simpleclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("simpleclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.simpleclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.simpleclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("simpleclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("simpleclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("simpleclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("simpleclaw gateway probe")}`);
  }
}
