...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: SimpleClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: SimpleClawConfig) => { changed: boolean; next: SimpleClawConfig };
  defaultModel: string;
  overrideConfig: SimpleClawConfig;
  alreadyDefaultConfig: SimpleClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: SimpleClawConfig) => config,
      applyDefaultConfig: (config: SimpleClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: SimpleClawConfig = { agents: { defaults: {} } };
