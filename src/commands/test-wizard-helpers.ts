...
export async function setupAuthTestEnv(
  ...
  process.env.SIMPLECLAW_AGENT_DIR = agentDir;
...
export function requireSimpleClawAgentDir(): string {
  const agentDir = process.env.SIMPLECLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SIMPLECLAW_AGENT_DIR not set");
  }
  return agentDir;
}
