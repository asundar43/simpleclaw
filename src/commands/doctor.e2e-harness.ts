...
export const resolveSimpleClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadSimpleClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/simpleclaw.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveSimpleClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SIMPLECLAW_STATE_DIR;
  originalUpdateInProgress = process.env.SIMPLECLAW_UPDATE_IN_PROGRESS;
  process.env.SIMPLECLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "simpleclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SIMPLECLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SIMPLECLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.SIMPLECLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
