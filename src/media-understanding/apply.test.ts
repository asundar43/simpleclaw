...
const TEMP_MEDIA_PREFIX = "simpleclaw-media-";
...
function createGroqAudioConfig(): SimpleClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SimpleClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SimpleClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SimpleClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredSimpleClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        SIMPLECLAW_AGENT_DIR: isolatedAgentDir,
