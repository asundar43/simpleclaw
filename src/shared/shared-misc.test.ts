...
describe("shared/frontmatter", () => {
  ...
  test("resolveSimpleClawManifestBlock parses JSON5 metadata and picks simpleclaw block", () => {
    const frontmatter = {
      metadata: "{ simpleclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveSimpleClawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveSimpleClawManifestBlock returns undefined for invalid input", () => {
    expect(resolveSimpleClawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveSimpleClawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveSimpleClawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
