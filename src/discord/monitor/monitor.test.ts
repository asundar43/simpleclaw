...
describe("agent components", () => {
  const createCfg = (): SimpleClawConfig => ({}) as SimpleClawConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): SimpleClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/simpleclaw-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/simpleclaw",
    });
  });
});
