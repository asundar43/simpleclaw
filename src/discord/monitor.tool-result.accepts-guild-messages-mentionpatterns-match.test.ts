  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/simpleclaw",
      },
    },
    session: { store: "/tmp/simpleclaw-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bsimpleclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "simpleclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "SimpleClaw" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/simpleclaw" },
      session: { store: "/tmp/simpleclaw-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
