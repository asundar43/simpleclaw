...
function resolveVoiceTtsConfig(params: { cfg: SimpleClawConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredSimpleClawTmpDir(), "discord-voice-"));
