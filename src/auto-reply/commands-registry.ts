...
export function isCommandEnabled(cfg: SimpleClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SimpleClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SimpleClawConfig),
...
export function getCommandDetection(_cfg?: SimpleClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SimpleClawConfig) {
