...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: SimpleClawConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: SimpleClawConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.SIMPLECLAW_TEST_FAST === "1") {
  ...
  const skillEligibility = getSkillEligibilityContext();
