...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bsimpleclaw\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: { identityName: "SimpleClaw" },
        expected: "[SimpleClaw]",
      },
      {
      ...
      },
      {
        ...
        values: {
          identityName: "SimpleClaw",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[SimpleClaw] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
