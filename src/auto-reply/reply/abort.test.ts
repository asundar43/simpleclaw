...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop simpleclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@simpleclaw_bot", { botUsername: "simpleclaw_bot" })).toBe(
      true,
    );
    expect(isAbortRequestText("/Stop@simpleclaw_bot", { botUsername: "simpleclaw_bot" })).toBe(
      true,
