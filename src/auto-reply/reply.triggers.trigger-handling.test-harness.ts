...
type TempHomeEnvSnapshot = {
  ...
  simpleClawHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    simpleClawHome: process.env.SIMPLECLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("SIMPLECLAW_HOME", snapshot.simpleClawHome);
  restoreKey("SIMPLECLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.SIMPLECLAW_STATE_DIR = join(home, ".simpleclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "simpleclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".simpleclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): SimpleClawConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "simpleclaw"),
