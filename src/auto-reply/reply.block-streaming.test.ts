...
function createReplyConfig(home: string, streamMode?: "block"): SimpleClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("simpleclaw-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("SIMPLECLAW_TEST_FAST", "1");
