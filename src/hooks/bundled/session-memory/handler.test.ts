...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => SimpleClawConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("simpleclaw-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): SimpleClawConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
