...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SIMPLECLAW_BUNDLED_HOOKS_DIR"]);
    process.env.SIMPLECLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
