...
function resolveGatewayPort(cfg: SimpleClawConfig, env: NodeJS.ProcessEnv): number {
  const envRaw = env.SIMPLECLAW_GATEWAY_PORT?.trim() || env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
function resolveAuth(cfg: SimpleClawConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.SIMPLECLAW_GATEWAY_PASSWORD?.trim() ||
