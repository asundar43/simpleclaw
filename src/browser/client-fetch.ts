...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-simpleclaw-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-simpleclaw-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-simpleclaw-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the SimpleClaw gateway (SimpleClaw.app menubar, or \`${formatCliCommand("simpleclaw gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the SimpleClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the SimpleClaw browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
