...
const RELAY_AUTH_HEADER = "x-simpleclaw-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/SimpleClaw-Extension-Relay",
          revision: "0",
          userAgent: "SimpleClaw-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "SimpleClaw/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedSimpleClawRelay({
