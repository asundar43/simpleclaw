...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".simpleclaw-profile-decorated");
}
 ...
 */
export function decorateSimpleClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SIMPLECLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SIMPLECLAW_BROWSER_COLOR).toUpperCase();
