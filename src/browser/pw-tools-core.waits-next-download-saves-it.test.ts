...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSimpleClawTmpDir: vi.fn(() => "/tmp/simpleclaw"),
}));
vi.mock("../infra/tmp-simpleclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSimpleClawTmpDir.mockReturnValue("/tmp/simpleclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSimpleClawTmpDir.mockReturnValue("/tmp/simpleclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "simpleclaw-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "simpleclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSimpleClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "simpleclaw-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "simpleclaw-preferred", "downloads")}${path.sep}`),
    );
  });
