import type { BrowserConfig, BrowserProfileConfig, SimpleClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SIMPLECLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "simpleclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SIMPLECLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "simpleclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SIMPLECLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SIMPLECLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SimpleClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openclaw/simpleclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SimpleClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SIMPLECLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "simpleclaw";
