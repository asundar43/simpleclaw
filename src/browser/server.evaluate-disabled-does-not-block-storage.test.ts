  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          simpleclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SIMPLECLAW_GATEWAY_PORT;
    process.env.SIMPLECLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SIMPLECLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SIMPLECLAW_GATEWAY_PASSWORD;
