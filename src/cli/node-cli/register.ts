    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "simpleclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["simpleclaw node status", "Check node host service status."],
          ["simpleclaw node install", "Install the node host service."],
          ["simpleclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.simpleclaw.ai/cli/node")}\n`,
    );
