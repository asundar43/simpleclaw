...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SimpleClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["simpleclaw update", "Update a source checkout (git)"],
        ["simpleclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["simpleclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["simpleclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["simpleclaw update --dry-run", "Preview actions without changing anything"],
        ["simpleclaw update --no-restart", "Update without restarting the service"],
        ["simpleclaw update --json", "Output result as JSON"],
        ["simpleclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["simpleclaw update wizard", "Interactive update wizard"],
        ["simpleclaw --update", "Shorthand for simpleclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run simpleclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.simpleclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["simpleclaw update status", "Show channel + version status."],
          ["simpleclaw update status --json", "JSON output."],
          ["simpleclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.simpleclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
