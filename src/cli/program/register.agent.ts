...
${formatHelpExamples([
  ['simpleclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['simpleclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['simpleclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.simpleclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.simpleclaw.ai/cli/agents")}\n`,
    );
