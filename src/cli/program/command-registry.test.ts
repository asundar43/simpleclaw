...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "simpleclaw", "doctor"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "simpleclaw", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "simpleclaw", "status"], async () => {
      await program.parseAsync(["node", "simpleclaw", "status"]);
    });
