...
import type { SimpleClawConfig, ConfigFileSnapshot } from "../config/types.simpleclaw.js";
...
vi.mock("../infra/simpleclaw-root.js", () => ({
  resolveSimpleClawPackageRoot: vi.fn(),
}));
...
const { resolveSimpleClawPackageRoot } = await import("../infra/simpleclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/simpleclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as SimpleClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/simpleclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSimpleClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("simpleclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveSimpleClawPackageRoot).mockClear();
    ...
    vi.mocked(resolveSimpleClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("simpleclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/simpleclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "simpleclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("SimpleClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SimpleClawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveSimpleClawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("simpleclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SIMPLECLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SIMPLECLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("simpleclaw-update-wizard");
    await withEnvAsync({ SIMPLECLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
