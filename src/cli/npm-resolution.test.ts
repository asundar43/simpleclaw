...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@simpleclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@simpleclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@simpleclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@simpleclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @simpleclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@simpleclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@simpleclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/simpleclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@simpleclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @simpleclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@simpleclaw/plugin-alpha@latest",
      true,
      "/tmp/simpleclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@simpleclaw/plugin-alpha@latest",
      installPath: "/tmp/simpleclaw/extensions/alpha",
      version: "1.2.3",
