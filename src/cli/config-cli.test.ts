...
import type { ConfigFileSnapshot, SimpleClawConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `simpleclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: SimpleClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: SimpleClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: SimpleClawConfig, config: SimpleClawConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: SimpleClawConfig = {
        agents: {
      ...
      const runtimeMerged: SimpleClawConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__SIMPLECLAW_REDACTED__");
    });
  });

  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: SimpleClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
