...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "simpleclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "simpleclaw-bundled-skills-test-"));
    process.env.SIMPLECLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
