...
const SIMPLECLAW_REPO_URL = "https://github.com/simpleclaw/simpleclaw.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "simpleclaw";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("simpleclaw@")) {
    return trimmed.slice("simpleclaw@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.SIMPLECLAW_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveSimpleClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SIMPLECLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SIMPLECLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set SIMPLECLAW_GIT_DIR to an empty folder or an simpleclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SIMPLECLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "simpleclaw.mjs");
