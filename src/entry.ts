...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "simpleclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "simpleclaw.js", entryBasename: "entry.js" },
  { wrapperBasename: "simpleclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "simpleclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (
      isTruthyEnvValue(process.env.SIMPLECLAW_NO_RESPAWN) ||
      isTruthyEnvValue(process.env.OPENCLAW_NO_RESPAWN)
    ) {
      return false;
    }
    if (
      isTruthyEnvValue(process.env.SIMPLECLAW_NODE_OPTIONS_READY) ||
    ...
    process.env.SIMPLECLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[simpleclaw] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[simpleclaw] ${parsed.error}`);
      process.exit(2);
    }
    ...
    import("./cli/run-main.js")
      ...
      .catch((error) => {
        console.error(
          "[simpleclaw] Failed to start CLI:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
