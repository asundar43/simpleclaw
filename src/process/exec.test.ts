...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ SIMPLECLAW_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.SIMPLECLAW_BASE_ENV ?? "") + "|" + (process.env.SIMPLECLAW_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { SIMPLECLAW_TEST_ENV: "ok" },
        },
      );
