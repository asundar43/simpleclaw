...
export function resolveConfigDir(
  ...
  const override = env.SIMPLECLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".simpleclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SIMPLECLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SIMPLECLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SIMPLECLAW_STATE_DIR.
