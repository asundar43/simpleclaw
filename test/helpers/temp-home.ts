...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openclawHome: process.env.SIMPLECLAW_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SIMPLECLAW_HOME", snapshot.openclawHome);
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SIMPLECLAW_HOME.
  delete process.env.SIMPLECLAW_HOME;
  process.env.SIMPLECLAW_STATE_DIR = path.join(base, ".simpleclaw");
...
export async function withTempHome<T>(
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".simpleclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
