...
import type { SimpleClawPluginApi, SimpleClawPluginService } from "simpleclaw/plugin-sdk";
...
function normalizeDenyList(cfg: SimpleClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SimpleClawPluginApi["config"]): string[] {
...
export default function register(api: SimpleClawPluginApi) {
  ...
  const timerService: SimpleClawPluginService = {
