...
function fakeApi(overrides: Partial<SimpleClawPluginApi> = {}): SimpleClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(
  overrides: Partial<SimpleClawPluginToolContext> = {},
): SimpleClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: SimpleClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
