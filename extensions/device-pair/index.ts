...
function resolveGatewayPort(cfg: SimpleClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.SIMPLECLAW_GATEWAY_PORT?.trim()) ??
...
function resolveScheme(
  cfg: SimpleClawPluginApi["config"],
...
function resolveAuth(cfg: SimpleClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: SimpleClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the SimpleClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the SimpleClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
