...
vi.mock("simpleclaw/plugin-sdk", async () => {
  const actual =
    await vi.importActual<typeof import("simpleclaw/plugin-sdk")>("simpleclaw/plugin-sdk");
  return {
    ...actual,
...
import type { SimpleClawPluginServiceContext } from "simpleclaw/plugin-sdk";
import { emitDiagnosticEvent } from "simpleclaw/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/simpleclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): SimpleClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("simpleclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("simpleclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("simpleclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("simpleclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("simpleclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("simpleclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("simpleclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("simpleclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("simpleclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("simpleclaw.webhook.processed");
    expect(spanNames).toContain("simpleclaw.message.processed");
    expect(spanNames).toContain("simpleclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["simpleclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("simpleclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "simpleclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["simpleclaw.reason"]).toBe("string");
    expect(String(attrs?.["simpleclaw.reason"])).not.toContain(
