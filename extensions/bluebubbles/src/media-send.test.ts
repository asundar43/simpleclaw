...
import type { SimpleClawConfig, PluginRuntime } from "simpleclaw/plugin-sdk";
...
function createConfig(overrides?: Record<string, unknown>): SimpleClawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("sendBlueBubblesMedia local-path hardening", () => {
  ...
  it("rejects relative mediaLocalRoots entries", async () => {
    ...
    // Use an explicitly relative path instead of path.relative(), which can
    // return an absolute path on Windows when cwd and tmpdir are on different drives.
    await expect(
      sendBlueBubblesMedia({
        cfg: createConfig({ mediaLocalRoots: ["relative/root"] }),
        to: "chat:123",
        mediaPath: allowedFile,
      }),
