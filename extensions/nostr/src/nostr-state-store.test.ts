...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SIMPLECLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "simpleclaw-nostr-"));
  process.env.SIMPLECLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override =
          stateEnv.SIMPLECLAW_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SIMPLECLAW_STATE_DIR = previous;
    }
