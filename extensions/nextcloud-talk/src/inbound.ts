...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as SimpleClawConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as SimpleClawConfig,
    surface: CHANNEL_ID,
  });
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as SimpleClawConfig);
