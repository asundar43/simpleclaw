...
function resolveSimpleClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SIMPLECLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SIMPLECLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSimpleClawRoot(), "dist", "extensionAPI.js");
